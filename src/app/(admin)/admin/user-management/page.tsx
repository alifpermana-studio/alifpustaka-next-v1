...
import { BulkActionConfirmModal } from "@/components/user-management/BulkActionConfirmModal";
...
export default function UserManagementPage() {
  ...
  const [filter, setFilter] = useState<FilterState>({ search: "", role: "", status: "" });
  ...
  const fetchUsers = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showNotification(result.error?.message || "Failed to fetch users", "error");
      }
    } catch (error) {
  ...
  const handleSaveUser = async (userId: string, updates: { role?: UserRole; status?: UserStatus }) => {
    try {
      ...
      if (result.success) {
        ...
        showNotification(result.error?.message || "Failed to update user", "error");
      }
    } catch (error) {
  ...
  const handleBulkActionClick = (action: "activate" | "deactivate" | "ban") => {
    const editableUserIds = Array.from(selectedUsers).filter((userId) => {
      ...
      return userItem && canManageUser(userItem.role) && userItem.status !== "deleted";
    });

    if (editableUserIds.length === 0) {
      showNotification("No users selected that you have permission to edit", "warning");
      return;
    }

    setBulkAction({ action, userIds: editableUserIds, count: editableUserIds.length });
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-base-content/70 mt-1">
          Manage user roles and status. Last updated: {lastUpdated.toLocaleTimeString()}
        </p>
      </div>

      <UserFilters filter={filter} onFilterChange={setFilter} totalUsers={pagination.total} />
            ...
            onSelectAll={(checked) => {
              if (checked) {
                const selectableIds = users
                  .filter((u) => canManageUser(u.role) && u.status !== "deleted")
                  .map((u) => u.id);
