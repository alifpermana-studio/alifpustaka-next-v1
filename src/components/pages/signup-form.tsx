...
export const SignUpForm = () => {
  ...
  const handleSignUp = async (event: { preventDefault: () => void }) => {
    ...
    try {
      ...
      router.push(
        `/verify-email?e=${encodeURIComponent(data.user.email)}&br=signup`,
      );
    } catch (error: unknown) {
      console.log("Error signed up: ", error);
  ...
  useEffect(() => {
    ...
    if (!errorCheck.success) {
      ...
      (Object.keys(signupForm) as Array<keyof typeof signupForm>).forEach(
        (field) => {
          if (signupForm[field] === "") {
          ...
          <form onSubmit={handleSignUp} className="space-y-5">
            <div>
              ...
              <div className="group relative">
                <IdCard className="pointer-events-none absolute top-1/2 left-3.5 h-4.5 w-4.5 -translate-y-1/2 text-slate-500 transition-colors group-focus-within:text-blue-400" />
                <input
                  id="display-name"
                  type="text"
                  required
              ...
              <div
                className={`${formError.displayName.error ? "flex" : "hidden"} mt-2 flex-row items-center gap-2 text-sm text-red-400`}
              >
            ...
            <div>
              ...
              <div className="group relative">
                <UserRound className="pointer-events-none absolute top-1/2 left-3.5 h-4.5 w-4.5 -translate-y-1/2 text-slate-500 transition-colors group-focus-within:text-blue-400" />
                <input
                  id="username"
                  type="text"
                  required
              ...
              <div
                className={`${formError.username.error ? "flex" : "hidden"} mt-2 flex-row items-center gap-2 text-sm text-red-400`}
              >
                <TriangleAlert className="mr-1 inline h-4 w-4" />
