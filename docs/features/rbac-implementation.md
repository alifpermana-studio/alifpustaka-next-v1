...
- **Sales Admin** - Sales management features (with manage_sales permission)
- **Support Admin** - Support management features (with manage_support permission)
...
- `GET /api/users` - List users (User Admin, Super Admin)
  - Query params: `search`, `role`, `status`, `skip`, `limit`
  - Returns paginated user list with metadata
- `PATCH /api/users` - Update user role/status
  - Validates permissions with `canAssignRole()` and `canManageUser()`
  - Creates audit logs for changes
- `GET /api/users/[id]` - Get user details
- `GET /api/users/[id]/audit-logs` - Get user audit logs

**UI Component:** `/admin/user-management` - Full user management interface with search, filters, bulk actions
